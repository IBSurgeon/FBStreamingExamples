...
Обратите внимание: здесь происходит дублирование файлов архивов журналов, чтобы одновременно работала логическая репликация и задача по публикации событий в формате JSON.
...
Прежде всего скачиваем плагин `simple_json_plugin` и размещаем `simple_json_plugin.dll` в директории `$(fb_streaming_root)/stream_plugins`, где `$(fb_streaming_root)` - корневая директория установки службы `fb_streaming`.
...
С этого момента служба `fb_streaming` будет отслеживать изменения в указанных таблицах и публиковать изменения в json файлах, которые будут располагаться в директории, указанной в параметре `outputDir`.
